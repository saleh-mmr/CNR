...
class Trainer:
    ...
    def train(self):
        ...
        for episode in range(1, self.max_episodes + 1):
            cart_pole_env = self.cart_pole_env
            mountain_car_env = self.mountain_car_env
            # Initial observation from environment
            state_cp = cart_pole_env.reset()
            state_mc = mountain_car_env.reset()
            ...
            while not (done_cp and done_mc):
                ...
                if not done_cp:
                    ...
                    next_state_cp, reward_cp, done_cp = cart_pole_env.step(action_cp)
                ...
                if not done_mc:
                    ...
                    next_state_mc, reward_mc, done_mc = mountain_car_env.step(action_mc)
                ...
                for ap_index in range(len(self.envs)):
                    if len(self.agent.replay_memory[ap_index]) > self.batch_size:
                        self.agent.learn(self.batch_size, ap_index)
