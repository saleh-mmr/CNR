...
class ManhattanWeightController:
    ...
    def step(self):
        ...
        for name, st in self.state.items():
            ...
            # Skip NaN/Inf elements only
            valid = torch.isfinite(grad)

            pos = (grad > 0) & valid   # B1: grad>0 => decrease W => increase G-
            neg = (grad < 0) & valid   # B1: grad<0 => increase W => increase G+

            if pos.any():
                st["g_minus_idx"][pos] += 1
                st["g_minus_idx"].clamp_(0, max_idx)

            if neg.any():
                st["g_plus_idx"][neg] += 1
                st["g_plus_idx"].clamp_(0, max_idx)

            # Update conductances from indices
            values_dev = self._values_on(param.device, param.dtype)
            st["g_plus"].copy_(values_dev[st["g_plus_idx"]])
            st["g_minus"].copy_(values_dev[st["g_minus_idx"]])

            # Write back weight: W = G+ - G-
