...
class DQNAgent:
    def __init__(
        ...
        self.optimizer = RMSprop(self.q_network.parameters(), lr=3e-5)
    ...
    def learn(self, batch_size):
        if len(self.cart_pole_memory) < batch_size:                # Not enough samples in replay => Skip learning
            return None

        # Pulls a random batch from replay memory for training
        states, actions, next_states, rewards, dones = self.cart_pole_memory.sample(batch_size)
